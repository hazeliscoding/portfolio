...
- **Nothing under `src/app/features/` gains animation code**, with one narrow exception. If a page component needs motion, the motion belongs in the shared component it renders — that is what keeps animation reviewable in one place instead of scattered across six routes. The exception is an animated element that is **page-specific furniture no shared component would ever own**: M7's REC lamp on the project detail page, and M8's row feedback. Extracting a component for a blinking dot would cost more than the rule saves. Where the exception applies, the element still carries its own reduced-motion rule, at specificity matching the rule that set the animation — the exception is about *where* the CSS lives, never about whether it is complete.
