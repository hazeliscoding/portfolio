...
- **Nothing under `src/app/features/` gains animation code**, with one narrow exception. If a page component needs motion, the motion belongs in the shared component it renders — that is what keeps animation reviewable in one place instead of scattered across six routes. The exception covers two things. First, an animated element that is **page-specific furniture no shared component would ever own** — M7's REC lamp on the project detail page. Second, **transitions on an existing interactive state** of a page's own elements, whether that state is driven by hover or by selection: M8 touches both, and the distinction is not worth a second carve-out. Extracting a component for a blinking dot would cost more than the rule saves. Where the exception applies, the element still carries its own reduced-motion rule, at specificity matching the rule that set the animation — the exception is about *where* the CSS lives, never about whether it is complete.
